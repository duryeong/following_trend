...
def get_binance_recommendations(selected_date=None):
    try:
        # 캐시된 데이터 사용
        binance_info = load_binance_info()
        binance_info = binance_info.head(30)
        # binance_info = binance_info.sort_values(by='best_value', ascending=False).head(30)
        
        recommended_binances = []
        total_binances = len(binance_info)
        ...
        for idx, (_, row) in enumerate(binance_info.iterrows()):
            ticker = row['tickers']
            if row['best_param'] == '': continue
            try:
                status_text.text(f'분석 중: {ticker} ({idx+1}/{total_binances})')
                progress_bar.progress((idx + 1) / total_binances)
                
                odir = "binance_data"
                ofile = f"{odir}/{ticker.replace('/','_')}.pickle"
                ...
                if os.path.exists(ofile):
                    ...
                    df = binance.fetch_ohlcv(ticker, timeframe='1d', limit=180)  # OHLCV 데이터 가져오기
...
def main():
    ...
    with st.spinner('분석 중...'):
        recommended_binances = get_binance_recommendations(selected_date)
        
        if recommended_binances:
            st.success(f'분석 완료! 추천 종목 수: {len(recommended_binances)}개')
